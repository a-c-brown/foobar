...
public class SolutionTest {
    ...
    public void parseTriplesShouldReturnSixTriples() {
        ...
        Assert.assertEquals(6,
                Solution.parseTriples(trip1, new int[]{2, 4, 8, 16, 32}).size());

        List<List<Integer>> trip2 = new ArrayList<>();
        Assert.assertEquals(0,
                Solution.parseTriples(trip2, new int[]{3, 7, 11}).size());

        List<List<Integer>> trip3 = new ArrayList<>();
        Assert.assertEquals(1,
                Solution.parseTriples(trip3, new int[]{1, 1, 1}).size());
    }
    ...
    public void parseTriplesFromIndexShouldReturnExpected() {
        ...
        Assert.assertEquals(3,
                Solution.parseTriplesFromFixedIndex(trip1, new int[]{2, 4, 8, 16, 32}, 4).size());

        List<List<Integer>> trip2 = new ArrayList<>();
        Assert.assertEquals(2,
                Solution.parseTriplesFromFixedIndex(trip2, new int[]{2, 4, 8, 16}, 3).size());

        List<List<Integer>> trip3 = new ArrayList<>();
        Assert.assertEquals(1,
                Solution.parseTriplesFromFixedIndex(trip3, new int[]{2, 4, 8}, 2).size());
    }
    ...
    public void addTripleToSetShouldAddFirstFoundTriple() {
        ...
        Assert.assertEquals(expectedTriples,
                Solution.addTripleToSet(triples, new int[]{4, 2, 6, 8, 2, 6, 12}, 6, 5));
    }

    @Test
    public void addTripleToSetShouldFindNoTriples() {
        Assert.assertEquals(0, Solution.addTripleToSet(triples,
                new int[]{3, 7},
                1,
                0).size());
    }
